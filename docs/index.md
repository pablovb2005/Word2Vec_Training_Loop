# Documentation Hub

This is the canonical documentation for the final project submission.

Start here:
- [Quickstart](quickstart.md)
- [Architecture Overview](architecture/overview.md)
- [Training and Model Details](architecture/training-and-model.md)
- [CLI Usage](running/cli.md)
- [Scripts](running/scripts.md)
- [Makefile Commands](running/makefile.md)
- [Docker Usage](running/docker.md)
- [Quality Gates](development/quality.md)
- [Benchmarking](development/benchmarking.md)
- [Model Persistence](development/persistence.md)
- [Artifact Format Reference](reference/artifacts.md)
- [Benchmark Profiles Reference](reference/benchmark-profiles.md)

Recommended reading order:
1. Quickstart
2. CLI Usage
3. Benchmarking
4. Model Persistence
5. Architecture
